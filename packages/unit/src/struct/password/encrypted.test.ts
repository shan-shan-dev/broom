...
describe("EncryptedPassword", () => {
	...
	describe("from(password)", async () => {
		describe(
			`it succeeds when password is valid, doesn't last longer than ${timeout}ms, and:`,
			async () => {
				...
				it(`encrypted password is a string with ${EncryptedPassword.LENGTH} characters length`, ({
					...
					expect(encrypted.toString()).toHaveLength(EncryptedPassword.LENGTH);
				});
		...
		describe(`succeed and doesn't last longer than ${timeout}ms:`, () => {
			it(
				...
				async ({ expect }) => {
					...
					await expect(encrypted.isMatch(password)).resolves.toBe(true);
				},
				{ timeout },
			);
			...
			it(
				...
				async ({ expect }) => {
					...
					await expect(encrypted.isMatch(password)).resolves.toBe(false);
				},
				{ timeout },
			);
		});
	...
	describe("static isValid(value)", () => {
		it("returns false when value is invalid", ({ expect }) => {
			expect(EncryptedPassword.isValid("asdads10230130123-1-2301230120312-31203-0123-1")).toBe(false);
		});

		it("returns true when value is valid", ({ expect }) => {
			expect(EncryptedPassword.isValid("$2b$10$nOUIs5kJ7naTuTFkBy1veuK0kSxUFXfuaOKdOKf9xYT0KKIGSJwFa")).toBe(
				true,
			);
		});
	});
	...
	describe("static from(Password)", () => {
		it("successfully creates self from the Password instance", ({ expect }) => {
			const password = Password.random();

			expect(EncryptedPassword.from(password)).resolves.not.toThrowError(ZodError);
		});
	});
});
