...
import { type AbsolutePath } from "@libs/path/util";
...
async function promptQuestions() {
	...
	for (const [variable, options] of getPromptOptions().entries()) {
		const answer: Record<keyof typeof CONFIG_SCHEMA, string> = await prompt(options);
...
function getPromptOptions() {
	const questions: Map<EnvironmentVariable, Parameters<typeof prompt>[0]> = new Map();
...
function getType(schema: ZodTypeAny): unknown {
	return schema._def?.typeName === "ZodDefault" ? getType(schema._def.innerType) : schema;
}
